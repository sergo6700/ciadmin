...
class Admin_categories extends CI_Controller
{
     ...
     * @return void        $this->load->model('categories_model');
    ...
    public function index()
    {
        ...
        if ($search_string !== false && $order !== false || $this->uri->segment(3) == true) {
            ...
            */
            ...
            if ($search_string) {
                if ($order) {
                    $data['categories'] = $this->categories_model->get_categories(null,$search_string, $order, $order_type, $config['per_page'], $limit_end);
                } else {
                    $data['categories'] = $this->categories_model->get_categories(null,$search_string, '', $order_type, $config['per_page'], $limit_end);
                }
            } else {
                if ($order) {
                    $data['categories'] = $this->categories_model->get_categories(null,'', $order, $order_type, $config['per_page'], $limit_end);
                } else {
                    $data['categories'] = $this->categories_model->get_categories(null,'', '', $order_type, $config['per_page'], $limit_end);
                }
            }
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if ($this->form_validation->run()) {
                $data_to_store = array(
                    ...
                    'manufacture_id' => $this->input->post('manufactore')
                );
